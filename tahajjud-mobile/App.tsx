...
import { View, StatusBar, LogBox, Platform, StyleSheet, Dimensions, Modal, DeviceEventEmitter, Animated as RNAnimated } from 'react-native';
...
function routeFromNotification(data: any): void {
  ...
  const navigate = () => {
    ...
    if (type === 'streak_paywall') {
      setTimeout(() => {
        import('./context/PurchasesContext').then(m => {
          // openPaywall is exposed via context — emit an event instead
...
const NebulaBackground = () => {
  ...
  // visually still dark, but ~40% cheaper to render every frame.
